...
If a text has a red background, <MyTooltip content="Hello!"><span className="px-2 p-1 bg-red-500/30 rounded-md font-bold">you can hover it!</span></MyTooltip>
...
It's actually an  
<MyTooltip content="Means application programming interface, imagine it as a bridge between two towns. You use this bridge to connect to the other town."><span className="px-2 p-1 bg-red-500/30 rounded-md font-bold">API</span></MyTooltip>,
...
<MyTooltip content="Low-Level means how close you are to pure machine code. OpenGL is low-level because it's close to the GPU code."><span className="px-2 p-1 bg-red-500/30 rounded-md font-bold">low-level</span></MyTooltip>,
